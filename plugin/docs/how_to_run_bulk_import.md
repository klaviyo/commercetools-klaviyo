...
> **_NOTE:_** While `CT_SCOPE` is not mandatory for deployment, the listed scopes are **required for the component to function correctly**. The `manage_subscriptions` scope is **not required** for bulk import functionality. However, if you are sharing the same commercetools credentials with the realtime component, you should include `manage_subscriptions` in the scope to prevent post-deploy failures related to insufficient scope when making requests to commercetools.
...
| CT_SCOPE         | *commercetools API client scopes. The following scopes are required for the bulk import module to function correctly:* <br /> `view_orders` `view_published_products` `view_products` `manage_key_value_documents` `view_customers` `view_payments` | No | `view_orders:project-key view_published_products:project-key view_products:project-key manage_key_value_documents:project-key view_customers:project-key view_payments:project-key` |
