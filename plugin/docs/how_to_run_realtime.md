...
> **_NOTE:_** While `CT_SCOPE` is not mandatory for deployment, the listed scopes (including `manage_subscriptions`) are **required for the component to function correctly** if you decide to specify scopes. The `manage_subscriptions` scope is specifically needed because the post-deploy script (for Connect) creates and manages commercetools subscriptions that enable the real-time data sync functionality.
> While specifying the scope in your environment variables is not required, you do need to make sure your API credentials are created with the necessary scopes.
...
| CT_SCOPE         | *commercetools API client scopes. The following scopes are required for the realtime event plugin to function correctly:* <br /> `view_orders` `view_published_products` `view_products` `view_customers` `view_payments` `manage_subscriptions` | Yes | `view_orders:project-key view_published_products:project-key view_products:project-key view_customers:project-key view_payments:project-key manage_subscriptions:project-key` |
